...
public class frmHuesped extends javax.swing.JInternalFrame {
    String filtro="";
    String [][] Inquilinos = new String [1][10];
    static int aux=0;
    /**
    ...
    public void GenerarFactura(){
        ...
        String cuenta_Inquilino = "";
        cuenta_Inquilino = String.valueOf(txtCodigo.getText() + ".doc");
        File fichero = new File (cuenta_Inquilino);
        if(fichero.delete())
        ...
        {
            JOptionPane.showMessageDialog(null,"Debe digilenciar una identificación");
        }
        if(!"".equals(txtCodigo.getText()))
        {
            try
            {
             ...
             File archivo = new File(cuenta_Inquilino);   
                
             try(
                  FileWriter escribir  = new FileWriter(archivo,true)){
                  PrintWriter espacio = new PrintWriter(escribir);
                  escribir.write("**********Hotel CRUD************");
                  espacio.println();
                  espacio.println();
                  escribir.write("Codigo:"+ Inquilinos[aux][0]);
                  espacio.println();
                  espacio.println();
                  escribir.write("**********Datos del inqulino************");
                  espacio.println();
                  espacio.println();
                  escribir.write("Identificacion:"+ Inquilinos[aux][1]+ "\t" + "Nombre:"+Inquilinos[aux][2]);
                  espacio.println();
                  espacio.println();
                  escribir.write("**********Datos de la reserva************");
                  espacio.println();
                  espacio.println();
                  escribir.write("N_Habitacion:"+ Inquilinos[aux][3] + "\t" + "Tipo Habitacion:"+ Inquilinos[aux][4]);
                  espacio.println();
                  espacio.println();
                  escribir.write("Fecha De Ingreso::"+ Inquilinos[aux][6]);
                  espacio.println();
                  espacio.println();
                  escribir.write("Fecha De Salida:"+ Inquilinos[aux][7]);
                  espacio.println();
                  espacio.println();
                  escribir.write("Noches Hospedadas:"+ Inquilinos[aux][8]);
                  espacio.println();
                  espacio.println();
                  escribir.write("Precio por noche:"+ Inquilinos[aux][5]);
                  espacio.println();
                  espacio.println();
                  escribir.write("Total A Pagar:"+ Inquilinos[aux][9]);
                  espacio.println();
                  espacio.println();
                  escribir.write("**********Gracias Quiero mi 5.0************");
                  espacio.println();
                  espacio.println();
             }
      }
    ...
    Inquilinos [aux][0]= txtCodigo.getText();
    Inquilinos [aux][1]= txtId.getText();
    Inquilinos [aux][2]= txtNom.getText();
    Inquilinos [aux][3]= txtNHabitacion.getText();
    Inquilinos [aux][4]= txtTipo.getText();
    Inquilinos [aux][5]= txtPrecio.getText();
    Inquilinos [aux][6]= ((JTextField)dateIngreso.getDateEditor().getUiComponent()).getText();
    Inquilinos [aux][7]= ((JTextField)dateSalida.getDateEditor().getUiComponent()).getText();
    Inquilinos [aux][8]= txtNoche.getText();
    Inquilinos [aux][9]= txtTotal.getText();
    GenerarFactura();
    JOptionPane.showMessageDialog(null,"Factura Generado");
    btnImprimir.setEnabled(false);
    btnModificar.setEnabled(false);
    btnDesocupar.setEnabled(false);
    btnRegistrar.setEnabled(true);
    ...
    private void btnModificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnModificarActionPerformed
        if (txtEstado.getText().equals("Disponible")) {
            if (txtNHabitacion.getText().equals(lblAux.getText())) {
                Coneccion cn = new Coneccion();
                Connection cnx = cn.conectar();

                try 
                {
                    PreparedStatement pst = cnx.prepareStatement("UPDATE huespedes SET Identificacion_Clientes=?, Numero_Habitaciones=?, Fecha_Ingreso=?, Fecha_Salida=?, Cant_Noches=?, Valor=? WHERE Codigo = ?");
                    pst.setString(1, txtId.getText());
                    pst.setString(2, txtNHabitacion.getText());
                    pst.setString(3, ((JTextField)dateIngreso.getDateEditor().getUiComponent()).getText());
                    pst.setString(4, ((JTextField)dateSalida.getDateEditor().getUiComponent()).getText());
                    pst.setString(4, txtNoche.getText());
                    pst.setDouble(4, Double.parseDouble(txtTotal.getText()));
                    pst.executeUpdate();
                    PreparedStatement pt = cnx.prepareStatement("UPDATE habitaciones SET Estado=? WHERE Numero = ?");
                    pt.setString(1, "Ocupado");
                    pt.setString(2, txtNHabitacion.getText());
                    pt.executeUpdate();
                ...
                try 
                {
                    ...
                    pst.executeUpdate();
                    PreparedStatement ps = cnx.prepareStatement("UPDATE habitaciones SET Estado=? WHERE Numero = ?");
                    ps.setString(1, "Disponible");
                    ps.setString(2, lblAux.getText());
                    ps.executeUpdate();
                    PreparedStatement pt = cnx.prepareStatement("UPDATE habitaciones SET Estado=? WHERE Numero = ?");
                    pt.setString(1, "Ocupado");
                    pt.setString(2, txtNHabitacion.getText());
            ...
            JOptionPane.showMessageDialog(null, "HABITACIÓN NO ESTA DISPONIBLE");
            limpiar();
        }
